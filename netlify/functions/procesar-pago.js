  ...
  try {
    ...
    const mpResponse = await fetch("https://api.mercadopago.com/v1/payments", {
      ...
      headers: {
        ...
        // Aquí Netlify inyecta tu token automáticamente de forma segura
    ...
    // Si Mercado Pago rechaza el cobro
    if (!mpResponse.ok) {
      console.error("Rechazo de Mercado Pago:", JSON.stringify(mpResult));
      return {
        statusCode: 400,
        body: JSON.stringify({ 
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({ error: "Error interno al procesar el pago" })
    };
  }
};
